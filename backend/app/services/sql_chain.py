...
def _build_schema_documentation() -> str:
    ...
    return (
        ...
        "If a question uses a subjective or policy term that has no column, flag, "
        "or clearly defined threshold anywhere in this schema (e.g. 'approved' vs "
        "'unapproved' discount, 'urgent', 'VIP', 'excessive', 'suspicious') — do "
        "NOT invent a proxy condition to approximate it (e.g. treating 'no linked "
        "promotion' as if it meant 'unapproved'). An invented mapping like that "
        "looks precise and returns a real-looking number, but it's answering a "
        "business rule this data was never given a way to represent, which is "
        "worse than not answering at all. In that situation, do not produce a "
        "SELECT query — respond only with a short comment (e.g. '-- this concept "
        "is not represented in the schema') so the caller can recognize this "
        "wasn't answered and fall back appropriately, rather than execute a guess."
    )
...
ANSWER_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "an ID column if no readable name exists for it. "
            "Any answer this long MUST start with a short `###` heading naming the "
            "topic — never a plain paragraph with no heading. And within the prose "
            "itself (not just the table), bold every specific figure and name as you "
            "mention it (e.g. \"**Karachi Central Distribution 1** led with **PKR "
            "4.88 million**\") — don't write a flat sentence of plain text carrying "
            "the actual data; it needs to visually stand out, not blend into a wall "
            "of text. Never put a name in quotation marks — whenever you'd naturally "
